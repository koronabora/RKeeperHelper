...
Settings::Settings()
{
	...
	rkBaths = "SELECT SUM(PAYSUM) FROM[RK7].[dbo].[VRK7CUBEVIEW3001] WHERE (CATEGPATH LIKE '%СПА%' OR CATEGPATH LIKE '%БАНЯ%') AND  CURRENCYTYPE<>'Неплательщики' AND SHIFTDATE='?';";
	...
	rkHotel = "SELECT SUM(PAYSUM) FROM[RK7].[dbo].[VRK7CUBEVIEW3001] WHERE CATEGPATH LIKE '%Отель%' AND  CURRENCYTYPE <> 'Неплательщики' AND SHIFTDATE='?';";
